...
class EnvSpec(object):
    ...
    def make(self, env_settings = {}):
        ...
        if self._entry_point is None:
            ...
            if "Custom" in self._env_name:
                env = cls(env_settings = env_settings, **self._kwargs)
            else:
...
class EnvRegistry(object):
    ...
    def make(self, id, env_settings = {}):
        logger.info('Making new env: %s', id)
        spec = self.spec(id)
        env = spec.make(env_settings = env_settings)
...
def make(id, env_settings = {}):
    return registry.make(id, env_settings = env_settings)
