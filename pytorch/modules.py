...
class Simple_Layer(nn.Module):
    ...
    def init_with_p_dict(self, p_dict):
        ...
        length = len(p_dict)
        if length == 1:
            new_b_core_0 = self.b_core[:1] + p_dict[0]
            if self.output_size > 1:
                self.b_core.data.copy_(torch.cat([new_b_core_0, self.b_core[1:]], 0).data)
            else:
                self.b_core.data.copy_(new_b_core_0.data)
        elif length == 2:
            new_b_core_0 = self.b_core[:1] * p_dict[1] + p_dict[0]
            new_W_core_0 = self.W_core[:,:1] * p_dict[1]
            if self.output_size > 1:
                self.W_core.data.copy_(torch.cat([new_W_core_0, self.W_core[:, 1:]], 1).data)
                self.b_core.data.copy_(torch.cat([new_b_core_0, self.b_core[1:]], 0).data)
            else:
                self.W_core.data.copy_(new_W_core_0.data)
                self.b_core.data.copy_(new_b_core_0.data)
        else:
            raise Exception("The length of p_dict for Simple_Layer must be 1 or 2!")


    def init_bias_with_input(self, input, mode = "std_sqrt"):
        if not (mode is None or mode == "None"):
            output = input
            if hasattr(self, "input_size_original"):
                output = output.view(-1, self.input_size)
            output = torch.matmul(output, self.W_core)
            output_mean = output.mean().data[0]
            output_std = output.std().data[0]
            if mode == "std-sqrt":
                output_std_used = output_std / np.sqrt(self.output_size)
            elif mode == "std":
                output_std_used = output_std
            elif mode == "sqrt-output":
                output_std_used = np.sqrt(self.output_size)
            elif mode == "std-unit":
                output_std_used = 1
            elif mode == "std-zero":
                output_std_used = 1e-7
            else:
                raise Exception("mode {0} not recognized!".format(mode))
            self.b_core.data.normal_(mean = -output_mean, std = output_std_used)
    ...
    def standardize(self, mode = "b_mean_zero"):
        if mode == "b_mean_zero":
            b_mean = self.b_core.mean().data[0]
